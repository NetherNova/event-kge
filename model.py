...
def rank_left_fn_idx(simfn, embeddings_ent, embeddings_rel, leftop, rightop, inpr, inpo):
    ...
    if simfn == l2_similarity:
        ...
        expanded_lhs = tf.expand_dims(expanded_lhs, 3)
        expanded_lhs = tf.reduce_sum(tf.mul(expanded_lhs, rell), 2)
...
class TransH(object):
    ...
    def run(self, tg, sg, test_tg, test_size, num_steps, init_lr=1.0, skipgram=True, store_embeddings=False, lambd=0.05):
        ...
        with graph.as_default():
            ...
            # TransH Loss
            kg_loss = max_margin(simi, simin) + lambd * (tf.reduce_sum(tf.sqrt(tf.reduce_sum(self.E ** 2, axis=1)) - 1) +
                      tf.reduce_sum(dot(self.W, self.R) ** 2 /
...
class RESCAL(object):
    ...
    def run(self, tg, sg, test_tg, test_size, num_steps, init_lr=1.0, skipgram=True, store_embeddings=False, lambd=0.05):
        ...
        with graph.as_default():
            ...
            lhs = tf.reduce_sum(tf.mul(lhs, rell), 1)

            lhsn = tf.expand_dims(lhsn, 1)
            lhsn = tf.reduce_sum(tf.mul(lhsn, relln), 1)

            simi = tf.nn.sigmoid(dot(lhs, rhs))
            simin = tf.nn.sigmoid(dot(lhsn, rhsn))
