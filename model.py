...
def rank_left_fn_idx(simfn, embeddings_ent, embeddings_rel, leftop, rightop, inpr, inpo):
    ...
    # [embedding_size, 1, batch_size], [embedding_size, num_entities, batch_size]
    simi = tf.squeeze(tf.batch_matmul(batch_rhs, batch_lhs), 1)
    return simi
...
def rank_right_fn_idx(simfn, embeddings_ent, embeddings_rel, leftop, rightop, inpl, inpo):
    ...
    # [test_size, embedding_size], [embedding_size, num_entities]
    simi = simfn(leftop(lhs, rell), rhs)
    return simi
...
def incremental_concat_layer(embeddings, train_dataset, batch_size, embedding_size, len_sequence, num_words_per_sequence):
...
class TranslationEmbeddings(object):
    ...
    def run(self, tg, sg, test_tg, test_size, num_steps, init_lr=1.0, skipgram=True, store_embeddings=False):
        ...
        with graph.as_default():
            ...
            ranking_error_l = rank_left_fn_idx(self.fnsim, self.E, self.R, self.leftop, self.rightop, test_inpr, test_inpo)
            ranking_error_r = rank_right_fn_idx(self.fnsim, self.E, self.R, self.leftop, self.rightop, test_inpl, test_inpo)
            ...
            for b in range(1, num_steps + 1):
                ...
                if b % eval_step_size == 0:
                    ...
                    if b > 0:
                        ...
                        g_Es = g_Es / eval_step_size
                        g_Rs = g_Rs / eval_step_size
