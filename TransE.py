...
class TransE(object):
    ...
    def rank_left_idx_new(self, test_inpr, test_inpo, r_embs, ent_embs, c_embs, test_inpc):
        lhs = ent_embs
        results = np.zeros((len(test_inpr), ent_embs.shape[0]))
        for i, r in enumerate(test_inpo):
            rell = r_embs[r]
            lhs_ext = lhs[:, np.newaxis] + (c_embs[test_inpc[i]] * rell)
            rhs = ent_embs[test_inpr[i]]
            results[i] = -np.square(lhs_ext - rhs).sum(axis=2).transpose()
        return results

    def rank_right_idx_new(self, test_inpl, test_inpo, r_embs, ent_embs, c_embs, test_inpc):
        rhs = ent_embs
    ...
    def create_graph(self):
        ...
        self.C = tf.Variable(tf.random_uniform((self.len_hierarchy, 1), minval=-w_bound,
                                               maxval=w_bound), name="C")
        # TODO: constant range(1, n)...
        # learnable weight w
        ...
        self.inpc = tf.placeholder(tf.int32, [self.batch_size_kg], name="c")
        ...
        self.inpcn = tf.placeholder(tf.int32, [self.batch_size_kg], name="cn")
        ...
        self.test_inpc = tf.placeholder(tf.int32, [None], name="test_c")

        lhs = tf.nn.embedding_lookup(self.E, self.inpl)
        rhs = tf.nn.embedding_lookup(self.E, self.inpr)
        if len(self.subclass_constr) > 0:
            rell = tf.mul(tf.nn.embedding_lookup(self.C, self.inpc), tf.nn.embedding_lookup(self.R, self.inpo))
        else:
        ...
        if len(self.subclass_constr) > 0:
            relln = tf.mul(tf.nn.embedding_lookup(self.C, self.inpcn), tf.nn.embedding_lookup(self.R, self.inpon))
        else:
        ...
        if len(self.subclass_constr) > 0:
            ...
            kg_loss += self.lambd * tf.abs(tf.nn.l2_loss(supclass_types - subclass_types) - tf.nn.embedding_lookup(self.C, 1))
        ...
        self.loss = kg_loss
        if len(self.subclass_constr) > 0:
             self.loss += self.lambd * (tf.square(tf.nn.embedding_lookup(self.C, tf.constant(0)) - 1.0) +
                                  tf.nn.l2_loss(tf.nn.embedding_lookup(self.C, tf.constant([1,2,3,4,5,6]))))
        mu = tf.constant(1.0)
