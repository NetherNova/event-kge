...
if __name__ == '__main__':
    ...
    path_to_kg = base_path + "Ontology/test_5.xml"
    path_to_store_sequences = base_path + "Sequences/"
    path_to_store_embeddings = base_path + "Embeddings/"
    traffic_data = False
    path_to_sequence = base_path + 'Sequences/sequence_5.txt'
    ...
    preprocessor = PreProcessor(path_to_kg)
    preprocessor.load_unique_msgs_from_txt(base_path + 'unique_msgs5.txt')
    amberg_params = None

    for event_layer in [ConcatenationCause]:
        for shared in [True]:
            # set fixed random seed
            rnd = np.random.RandomState(43)

            preprocessor.load_knowledge_graph(format='xml',  amberg_params=amberg_params, clean_schema=True)
            ...
            param_dict['learning_rate'] = [0.05, 0.1]     # [0.5, 0.8, 1.0]
            ...
            for comb_num, tmp_param_dict in enumerate(param_combs):
                ...
                if event_layer is not None:
                    if traffic_data:
                        ...
                        batch_size_sg = 32 # (sum([len(sequences[i]) for i in range(num_sequences)]) * num_epochs) / num_steps
